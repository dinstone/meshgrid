...
import com.dinstone.meshgrid.mesher.handler.AdminHandler;
...
public class HttpAdminVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        mainRouter.route().handler(TimeoutHandler.create(1000));

        mainRouter.route("/upline").handler(adminHandler::upline);
        mainRouter.route("/dwline").handler(adminHandler::dwline);
        mainRouter.route("/list").handler(adminHandler::services);
